...
**미포함.** 유저 스터디(향후 과제)와 실제 이미지 생성; 오프라인 analyzer는 키워드 기반 — 감정 게이트의 한계이기도 함: 의도적으로 감정 단어를 피한 패러프레이즈의 감정은 놓치며(Exp. 3 모호 조건), 논문의 GPT-4o analyzer를 통한 게이팅은 여기서 미검증. Exp. 3 시나리오는 소규모 수작업 셋(질의 n = 10) — Exp. 5가 공개 벤치마크(LoCoMo, 1,535 QA)로 보완하지만, 감정 현저성 라벨을 가진 공개 데이터셋은 없어 거기서의 E는 시스템 자체 analyzer 산출이며, 감정 재부상 주장 자체는 여전히 Exp. 3과 궁극적으로는 향후 유저 스터디에 의존.
